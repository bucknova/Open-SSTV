...
def test_incremental_vs_batch_byte_identical_scottie_s1() -> None:
    """ScottieS1IncrementalDecoder output is pixel-for-pixel identical to
    decode_wav on a clean (zero-noise) synthetic Scottie S1 signal.

    The batch decoder uses slant_corrected_line_starts (least-squares fit)
    while the incremental decoder uses walk_sync_grid (anchor-and-walk).
    On a clean synthetic signal with no clock drift both methods land on the
    same sync positions (within rounding), so the pixel values are identical.
    """
    fs = 48_000
    img = _make_gradient_256()
    audio = _encode_scottie_s1(img, fs)
    ...
    batch = decode_wav(audio, fs)
    assert batch is not None, "batch decode_wav returned None on clean signal"
    batch_pixels = np.array(batch.image)  # shape (256, 320, 3)
    ...
    inc_pixels = np.array(inc.get_image())  # shape (256, 320, 3)

    # Byte-identical: every pixel in every channel must match.
    mismatch = np.where(inc_pixels != batch_pixels)
    if len(mismatch[0]) > 0:
        rows = np.unique(mismatch[0])
        max_diff = int(
            np.max(np.abs(inc_pixels.astype(int) - batch_pixels.astype(int)))
        )
        pytest.fail(
            f"Incremental and batch pixel outputs differ on {len(mismatch[0])} "
            f"pixels across {len(rows)} rows. First mismatched row: {rows[0]}. "
            f"Max absolute pixel difference: {max_diff}. "
            f"Check that _sample_pixels_inc matches decoder._sample_pixels."
        )
